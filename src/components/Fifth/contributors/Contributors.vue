...
<script>
import { sponsors, volunteers, participantSponsors, regularParticipants } from '@/store/fifth/store';
import Sponsor from '@/components/Fifth/contributors/Sponsor';
import Volunteer from '@/components/Fifth/contributors/Volunteer';
  ...
  mounted() {
    ...
    window.onscroll = () => {
      if (this.checkVisible(contributorsElm, scrollDist + ExtraDist, scrollMode) && !alreadySeen) {
        ...
        // for (const i = 0; i < sponsorImages.length; i++) {
        //   this.lightUp(sponsorImages[i])
        // }
      }
    };
